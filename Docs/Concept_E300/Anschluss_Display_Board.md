...
## ST-Pin
Der ST-Pin ist für das Schalten des Standby Relais zuständig. Dieses unterbricht die Spannungsversorgung zum großen Trafo am Boden des Subwoofers. Wenn ST auf `HIGH` ist schaltet das Relais nicht durch. Wenn ST auf `LOW` ist schaltet das Relais durch und die beiden Amps werden über den großen Trafo mit Spannung versorgt.
...
Es reicht nicht ST direkt an einen ESP32 zu hängen und den GPIO-PIN auf `LOW` zu setzen. Dementsprechend funktionieren auch simple I2C Logic Level Shifter nicht, da diese nur das `LOW` Signal des Mikrocontrollers durchschalten und zusätzlich noch einen 10k Ohm Pull Up hinzufügt.
...
Wenn MUTE über einen GPIO Pin eines ESP32 auf 3,3V geschaltet wird, schält das Relais des Subwoofer Amps und entmutet in. Der AMP der Satelliten Lautsprecher ist dann jedoch unbrauchbar leise. Dies ist auch der Fall, wenn MUTE über einen 10k Ohm Pull Up auf 5V geschaltet wird. 
Dementsprechend funktionieren I2C Logic Level Shifter auch nicht, da diese nur einen 10K Ohm Pull Ip auf 5V verwenden.

MUTE wird nur korrekt als `HIGH` erkannt, wenn der Pin über einen 1k Ohm Pull Up oder direkt mit 5V verbunden wird. Siehe [Issue #3](https://github.com/CoderTobi/Teufel-Concept-E300-reverse-engineering/issues/3).
...
## C-D
Ich weiß nicht genau wofür `C-D` steht. Aber es scheint vom Display Board mit wahrscheinlich 10k Ohm auf 5V hochgezogen zu werden. Wenn aber das Display Board getrennt wird, dann liegen an `C-D` entweder 4,8V oder 0V an, je nach dem, ob gerade Musik abgespielt wird. Es wird dabei jedoch nur Audio beachtet, dass über die beiden vorderen Lautsprecher (FR und FL) abgespielt werden soll. 
